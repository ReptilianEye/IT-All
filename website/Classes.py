...
class TournamentController():
    ...
    def CreateNew(self, user_Id, name, date, time, location, partLimit, desripiton):
        self.tournament = Tournament(user_id=user_Id, name=name,
                                     date=date, time=time, location=location, max_part=partLimit, description=desripiton)
    ...
    def AddParticipant(self, user):
        db.session.add(Tournament(user_id=user.id, name=self.tournament.name,
                                  date=self.tournament.date, time=self.tournament.time,location=self.tournament.location,partNumber = self.tournament.partNumber+ 1,max_part = self.tournament.max_part, description=self.tournament.description))
        self.tournament.partNumber += 1
        db.session.commit()
        return self.tournament.id
