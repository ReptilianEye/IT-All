...
def getScheduleInfo():
    if request.method == 'POST':
        ...
        partLimit = request.form.get('partLimit')
        ...
        if len(name) < 3:
            ...
            flash(f'Select time when event is going to happend.', category='error')
        elif not location:
            flash(f"Type in tournament's location.", category='error')
        elif not partLimit:
            flash(f"Type in max participants.", category='error')
            ...
            id = newTournament.CreateNew(
                current_user.id, name, date, time, location, partLimit, description)
...
@views.route('/join-to-event', methods=['GET', 'POST'])
@login_required
def joinEvent():
    tournamentDTO = TournamentController()
    tournamentDTO.Load(current_user.current_tournament_id)
    tournamentDTO.AddParticipant(current_user)
    flash("Sucessfully joined to event", category='success')
    return render_template("tournaments.html", user=current_user)
...
@views.route('public-schedule', methods=['POST', 'GET'])
@login_required
def public_schedule():
    tournaments = Tournament.query.filter_by(is_public=True).all()
    if tournaments:
        return render_template("public_events.html", user=current_user, events=tournaments)
    else:
        flash('Unfortunately, there is no public schedule for now.', category='error')
        return redirect(url_for("views.home"))
...
def delete_tournament():
    ...
    if tournamentDTO.tournament:
        if tournamentDTO.tournament.user_id == current_user.id:
            ...
            tournamentDTO.tournament.partNumber -= 1
        tournamentDTO.Save()
